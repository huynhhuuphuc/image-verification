...
import { Calendar, Package, CheckCircle, AlertTriangle, TrendingUp, Filter, Menu } from 'lucide-react';
...
const HomeScreen: React.FC<HomeScreenProps> = ({ user, onToggleSidebar }) => {
  ...
  const successRate = Math.round((mockStats.successfulInspections / mockStats.totalInspections) * 100);
      ...
      <div className="mobile-container py-4 sm:py-6 max-w-7xl mx-auto pb-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 space-y-4 sm:space-y-0">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">
                Chào mừng trở lại, {user?.company || 'Saigonfood'}! 👋
              </h1>
        ...
        <div className="mobile-stats-grid mb-6 sm:mb-8">
          {stats.map((stat, index) => (
            <div key={index} className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 animate-fade-in"
              style={{ animationDelay: `${index * 0.1}s` }}>
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs sm:text-sm font-medium text-gray-600 mb-1">{stat.title}</p>
                  <p className="text-lg sm:text-2xl font-bold text-gray-900">{stat.value.toLocaleString()}</p>
                </div>
              ...
              <div className="mt-3 sm:mt-4 flex items-center">
                ...
                <span className="text-xs sm:text-sm text-green-600 font-medium">+5.2%</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
            <div className="flex items-center justify-between mb-4 sm:mb-6">
              <h3 className="text-base sm:text-lg font-semibold text-gray-900">Tỷ lệ thành công</h3>
              ...
              <div className="relative w-24 h-24 sm:w-32 sm:h-32">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    <div className="text-xl sm:text-2xl font-bold text-gray-900">{successRate}%</div>
            ...
            <div className="grid grid-cols-2 gap-4 text-center">
              <div>
                <div className="text-base sm:text-lg font-semibold text-green-600">{mockStats.successfulInspections}</div>
                <div className="text-xs sm:text-sm text-gray-500">Đạt yêu cầu</div>
              </div>
              <div>
                <div className="text-base sm:text-lg font-semibold text-red-600">{mockStats.errorInspections}</div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6">
            <div className="flex items-center justify-between mb-4 sm:mb-6">
              <h3 className="text-base sm:text-lg font-semibold text-gray-900">Sản phẩm lỗi nhiều nhất</h3>
            ...
            <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-amber-50 rounded-lg">
              <div className="flex items-start space-x-2">
                ...
                <div>
                  <p className="text-xs sm:text-sm font-medium text-amber-800">Khuyến nghị</p>
                  <p className="text-xs text-amber-700 mt-1">
                    Cần kiểm tra lại quy trình in ấn cho các sản phẩm có tỷ lệ lỗi cao
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 sm:p-6 mb-6">
          ...
          <div className="space-y-3">
            {[
              { time: '10:30', action: 'Kiểm tra thành công', product: 'Cháo Tôm Thịt 240g', status: 'success' },
              { time: '09:15', action: 'Phát hiện lỗi', product: 'Mì Gói Tôm Chua Cay', status: 'error' },
              { time: '08:45', action: 'Kiểm tra thành công', product: 'Tôm Khô Cao Cấp', status: 'success' }
            ].map((activity, index) => (
